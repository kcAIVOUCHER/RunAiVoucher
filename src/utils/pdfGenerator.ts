...
function applySmartPageBreaks(container: HTMLElement, pageHeightPx = 1122.94) {
  ...
  const existingSpacers = container.querySelectorAll(".pdf-smart-spacer, [data-pdf-spacer='true']");
  existingSpacers.forEach((el) => el.remove());

  // Professional layout safety margins for A4 pages
  // Breathing space at the top of subsequent pages (pages 2, 3, etc.)
  const PAGE_TOP_MARGIN = 32;
  // Safety buffer before the bottom cut boundary to avoid text/borders touching the page edge
  const PAGE_BOTTOM_MARGIN = 36;

  // Reliable element layout metrics relative to the container root
  const getLayoutMetrics = (el: HTMLElement) => {
    ...
    if (cRect.height > 0 && elRect.height > 0) {
      top = Math.round(elRect.top - cRect.top);
      height = Math.round(elRect.height);
    } else {
      // Robust layout traversal fallback (immune to off-screen / viewport constraints)
  ...
  const getPageBounds = (pageIndex: number) => {
    const pageTop = pageIndex * pageHeightPx;
    const pageBottom = (pageIndex + 1) * pageHeightPx;
    const usableTop = pageIndex === 0 ? 0 : pageTop + PAGE_TOP_MARGIN;
    const usableBottom = pageBottom - PAGE_BOTTOM_MARGIN;
    return { pageIndex, pageTop, pageBottom, usableTop, usableBottom };
  };

  // Inserts a flex-safe, non-collapsible spacer before targetEl so it begins precisely at targetTop
  const insertSpacerBefore = (targetEl: HTMLElement, targetTop: number) => {
    const currentMetrics = getLayoutMetrics(targetEl);
    const neededShift = targetTop - currentMetrics.top;
    ...
    if (parent) {
      ...
      gap = parseFloat(computed.rowGap || computed.gap || "0") || 0;
    }

    const hasPreviousSibling = Boolean(targetEl.previousElementSibling);
    let spacerHeight = hasPreviousSibling
      ? Math.max(1, Math.round(neededShift - gap))
      : Math.round(neededShift);
    ...
    spacer.style.cssText = `
      ...
      max-height: ${spacerHeight}px !important;
      flex: 0 0 ${spacerHeight}px !important;
    ...
    // Live layout post-verification: Ensure pixel-perfect placement on the target page
    const verifiedMetrics = getLayoutMetrics(targetEl);
    const errorDelta = targetTop - verifiedMetrics.top;
    if (Math.abs(errorDelta) > 1) {
      spacerHeight = Math.max(1, Math.round(spacerHeight + errorDelta));
      spacer.style.height = `${spacerHeight}px`;
      spacer.style.minHeight = `${spacerHeight}px`;
      spacer.style.maxHeight = `${spacerHeight}px`;
  ...
  // Collect primary sections/blocks in strict top-to-bottom document order
  const primaryElements = Array.from(contentWrapper.children).filter(
    (el): el is HTMLElement =>
      el instanceof HTMLElement &&
      !el.classList.contains("pdf-smart-spacer") &&
      el.tagName !== "STYLE"
  );

  // PASS 1: Sequential section & card break evaluation
  primaryElements.forEach((el) => {
    ...
    if (isSection) {
      ...
      if (cards.length > 0) {
        ...
        const anchorMetrics = getLayoutMetrics(anchorEl);
        const card0Metrics = getLayoutMetrics(cards[0]);

        const currentSectionPage = Math.floor(anchorMetrics.top / pageHeightPx);
        const sectionBounds = getPageBounds(currentSectionPage);

        // If Header + Card 0 do not fit together on the current page, push the entire section to next page!
        if (card0Metrics.bottom > sectionBounds.usableBottom && anchorMetrics.top > sectionBounds.usableTop + 8) {
          const nextPageBounds = getPageBounds(currentSectionPage + 1);
          insertSpacerBefore(el, nextPageBounds.usableTop);
        }

        // Rule 2: Subsequent Cards (Card 1, Card 2, ...) must each fit completely or move to next page
        for (let i = 1; i < cards.length; i++) {
          const card = cards[i];
          const cardMetrics = getLayoutMetrics(card);

          const cardPage = Math.floor(cardMetrics.top / pageHeightPx);
          const cardBounds = getPageBounds(cardPage);

          // If this card overflows the current page, move it completely to the next page
          if (cardMetrics.bottom > cardBounds.usableBottom && cardMetrics.top > cardBounds.usableTop + 8) {
            const nextCardPageBounds = getPageBounds(cardPage + 1);
            insertSpacerBefore(card, nextCardPageBounds.usableTop);
          }
        }
        return;
      }
    ...
    if (isAtomicBlock) {
      const blockMetrics = getLayoutMetrics(el);
      const blockPage = Math.floor(blockMetrics.top / pageHeightPx);
      const blockBounds = getPageBounds(blockPage);

      if (blockMetrics.bottom > blockBounds.usableBottom && blockMetrics.top > blockBounds.usableTop + 8) {
        const nextBlockPageBounds = getPageBounds(blockPage + 1);
        insertSpacerBefore(el, nextBlockPageBounds.usableTop);
      }
    }
  });

  // PASS 2: Comprehensive safety scan across ALL cards to ensure zero page-cut slicing
  const allCards = Array.from(
    container.querySelectorAll<HTMLElement>(".pdf-card, [data-pdf-block='true']")
  );

  allCards.forEach((card) => {
    // Avoid double-processing elements that are direct wrappers of other checked cards
    if (card.querySelector(".pdf-card, [data-pdf-block='true']")) return;

    const metrics = getLayoutMetrics(card);
    const pageIndex = Math.floor(metrics.top / pageHeightPx);
    const bounds = getPageBounds(pageIndex);

    if (metrics.bottom > bounds.usableBottom && metrics.top > bounds.usableTop + 8) {
      // Find the appropriate element to push (if card is card 0 of a section, push section header or card)
      const section = card.closest<HTMLElement>(".pdf-section, [data-pdf-section]");
      const isFirstCardInSection = section && section.querySelector(".pdf-card, [data-pdf-block='true']") === card;
      const targetToPush = isFirstCardInSection && section ? section : card;

      const nextPageBounds = getPageBounds(pageIndex + 1);
      insertSpacerBefore(targetToPush, nextPageBounds.usableTop);
    }
  });
}
...
async function generateFromDedicatedTemplate({
  ...
  try {
    ...
    await new Promise<void>((resolve) => {
      ...
      // Wait for React commit and DOM paint including async QR/Barcodes
      setTimeout(resolve, 450);
    });

    // 5. Ensure all images inside the stage are completely loaded (logos, barcodes, QR codes)
    ...
    // Give browser brief tick to compute final spacer offsets
    await new Promise((r) => requestAnimationFrame(r));
