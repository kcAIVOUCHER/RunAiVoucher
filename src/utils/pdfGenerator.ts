...
function applySmartPageBreaks(container: HTMLElement, pageHeightPx = 1122.94) {
  ...
  // Layout parameters for A4 pages
  // Breathing space at the top of subsequent pages (pages 2, 3, etc.)
  const PAGE_TOP_MARGIN = 24;
  // Safety buffer before the bottom cut boundary to avoid text hitting page edge
  ...
  // Inserts a flex-safe, non-collapsible spacer before targetEl so it begins at targetTop
  const insertSpacerBefore = (targetEl: HTMLElement, targetTop: number) => {
    ...
    const spacerHeight = hasPreviousSibling
  ...
  primaryElements.forEach((el) => {
    if (el.classList.contains("pdf-smart-spacer") || el.tagName === "STYLE") return;
    ...
    if (isSection) {
      ...
      if (cards.length > 0) {
        ...
        if (card0Metrics.bottom > sectionBounds.usableBottom) {
        ...
        for (let i = 1; i < cards.length; i++) {
          ...
          // If this card overflows the current page, move it to the next page
          if (
            cardMetrics.bottom > cardBounds.usableBottom &&
            cardMetrics.height <= pageHeightPx - (PAGE_TOP_MARGIN + PAGE_BOTTOM_MARGIN)
          ) {
    ...
    if (isAtomicBlock) {
      ...
      if (
        blockMetrics.bottom > blockBounds.usableBottom &&
        blockMetrics.height <= pageHeightPx - (PAGE_TOP_MARGIN + PAGE_BOTTOM_MARGIN)
      ) {
