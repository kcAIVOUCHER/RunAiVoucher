      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden border border-slate-200">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm">
          ...
          {(formData.serviceType === "flight" || formData.serviceType === "package" || formData.serviceType === "combo") && (
            ...
            {/* Controles de Visibilidade das Tarifas e Classes no Voucher & PDF */}
            <div className="flex flex-wrap items-center justify-between gap-2 p-3 bg-purple-50/60 rounded-xl border border-purple-200">
              <div className="text-xs font-bold text-purple-900 flex items-center gap-1.5">
                <span>Visibilidade no Voucher & PDF:</span>
              </div>
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={!formData.hideFareFamily}
                    onChange={(e) => setFormData({ ...formData, hideFareFamily: !e.target.checked })}
                    className="rounded text-purple-600 focus:ring-purple-500"
                  />
                  <span>Exibir Família Tarifária (Light, Plus...)</span>
                </label>
                <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={!formData.hideBookingClass}
                    onChange={(e) => setFormData({ ...formData, hideBookingClass: !e.target.checked })}
                    className="rounded text-sky-600 focus:ring-sky-500"
                  />
                  <span>Exibir Classes de Reserva (Y, Q...)</span>
                </label>
              </div>
            </div>
