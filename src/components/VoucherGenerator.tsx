...
export const VoucherGenerator: React.FC<VoucherGeneratorProps> = ({
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (oversizedFiles.length > 0) {
      setParseError(`O arquivo "${oversizedFiles[0].name}" é muito grande (${(oversizedFiles[0].size / (1024 * 1024)).toFixed(1)}MB). O limite máximo suportado é de 25MB por arquivo.`);
  ...
  const handleProcessWithGemini = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Resposta não-JSON recebida do servidor:", responseText.slice(0, 300));
        if (response.status === 413 || responseText.includes("PayloadTooLargeError") || responseText.includes("too large")) {
          throw new Error("O arquivo PDF ou imagem é muito pesado para o servidor. Tente enviar um arquivo menor que 25MB ou reduza a resolução/quantidade de páginas.");
        }
        if (response.status === 502 || response.status === 504) {
          throw new Error("O servidor demorou para responder ou está reiniciando. Por favor, aguarde alguns instantes e tente novamente.");
        }
        throw new Error(`Falha no servidor (${response.status || 500}). O servidor retornou uma página de erro em vez dos dados processados.`);
      }
      ...
      setCurrentVoucher((prev) => {
        ...
        return {
          ...
          // Códigos gráficos pertencem estritamente aos serviços específicos (voo, hotel, etc.) e nunca na raiz
          qrCodeData: undefined,
          barcodeData: undefined,
          barcodeType: undefined,
          codeImageBase64: undefined,
  ...
  const handleCopyWhatsApp = () => {
    ...
    if (currentVoucher.serviceType === "car" && currentVoucher.carRental) {
      details =
        ...
        `*Retirada:* ${currentVoucher.carRental.pickupLocation} em ${formatDateBR(currentVoucher.carRental.pickupDate)} às ${currentVoucher.carRental.pickupTime}\n` +
        `*Devolução:* ${currentVoucher.carRental.dropoffLocation} em ${formatDateBR(currentVoucher.carRental.dropoffDate)} às ${currentVoucher.carRental.dropoffTime}\n` +
      ...
      details =
        ...
        `*Check-in:* ${formatDateBR(currentVoucher.hotel.checkInDate)} | *Check-out:* ${formatDateBR(currentVoucher.hotel.checkOutDate)}\n` +
      ...
      details =
        ...
        currentVoucher.flights
          .map(
            (f) =>
              `✈️ *${f.airline} (${f.flightNumber})*: ${f.departureCode} (${f.departureTime}) ➔ ${f.arrivalCode} (${f.arrivalTime}) em ${formatDateBR(f.departureDate)}`
          )
          .join("\n");
    }
    ...
    <div className="space-y-8">
      ...
      <section className="no-print bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-5">
        <div className="border-b border-slate-100 pb-4">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Envie PDFs ou imagens dos seus fornecedores. A IA identifica e organiza automaticamente os serviços no voucher.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-2 border-t border-slate-100">
          ...
          <button
            ...
            className="px-6 py-2.5 text-white font-bold text-xs rounded-xl shadow-md hover:shadow-lg disabled:opacity-50 flex items-center gap-2 transition-all shrink-0 cursor-pointer active:scale-[0.99] min-h-[44px]"
            style={{
              background: "linear-gradient(135deg, #00277A 0%, #152A9D 40%, #16B5F0 100%)",
            }}
          >
            <Sparkles className="w-4 h-4 text-[#21D8FF]" />
            {isProcessing ? "Lendo e organizando..." : "✨ Ler e organizar"}
          </button>
        </div>
      </section>
      ...
      {!isVoucherReady ? (
        ...
        <section className="bg-white rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center shadow-xs">
          <div className="max-w-md mx-auto flex flex-col items-center">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-6">
              Anexe um ou mais comprovantes (PDF ou imagem) no campo acima ou cole os dados da reserva e clique em 
              <strong className="text-sky-700"> "✨ Ler e organizar"</strong>. 
              A pré-visualização completa em padrão A4 aparecerá aqui com todos os serviços unificados, pronta para conferir e gerar o voucher.
            </p>
        ...
        <>
          ...
          {(saveSuccessMsg || pdfSuccessMsg || copiedMsg) && (
            <div className="no-print flex flex-wrap items-center gap-2">
              {saveSuccessMsg && (
                <span className="text-xs font-bold text-emerald-800 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200 animate-fade-in flex items-center gap-1.5 shadow-2xs">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Salvo no Histórico com Sucesso!
                </span>
              )}
              {pdfSuccessMsg && (
                <span className="text-xs font-bold text-sky-800 bg-sky-50 px-3 py-1.5 rounded-lg border border-sky-200 animate-fade-in flex items-center gap-1.5 shadow-2xs">
                  <CheckCircle2 className="w-3.5 h-3.5 text-sky-600" /> Arquivo PDF Baixado!
                </span>
              )}
              {copiedMsg && (
                <span className="text-xs font-bold text-emerald-800 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200 animate-fade-in flex items-center gap-1.5 shadow-2xs">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Resumo Copiado para WhatsApp!
                </span>
              )}
            </div>
          )}
