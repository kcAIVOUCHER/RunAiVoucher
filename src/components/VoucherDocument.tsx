    ...
    >
      ...
      <div className="p-4 sm:p-5 md:p-6 space-y-3.5 print:p-2 print:space-y-2.5">
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {voucher.flights.map((flight, idx) => (
                <div
                  key={flight.id || idx}
                  className="pdf-block-avoid border border-slate-200 rounded-lg p-4 bg-white hover:border-slate-300 transition-colors shadow-xs"
                  data-pdf-block="true"
                >
                  ...
                  <div className="flex flex-col gap-0.5 py-2 text-xs">
                    ...
                    {/* QR Code / Código de Barras estritamente dentro do quadro do voo, somente se existir no comprovante original */}
                    {(flight.qrCodeData || flight.barcodeData || flight.codeImageBase64) && (
                      <div className="mt-2.5 pt-2 border-t border-slate-100 flex justify-end">
                        <CodeRenderer
                          ...
                          title={`Cartão de Embarque - Voo ${getFlightNumberDisplay(flight.airlineCode, flight.flightNumber)}`}
                          compact={true}
                        />
                      </div>
                    )}
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {hotelsList.map((hotel, index) => (
                <div
                  key={hotel.id || index}
                  className="pdf-block-avoid border border-slate-200 rounded-xl p-4 bg-white space-y-3 shadow-xs hover:border-slate-300 transition-colors"
                  data-pdf-block="true"
                >
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {carsList.map((car, index) => (
                <div
                  key={car.id || index}
                  className="pdf-block-avoid border border-slate-200 rounded-lg p-4 bg-slate-50/40 space-y-3 shadow-xs hover:border-slate-300 transition-colors"
                  data-pdf-block="true"
                >
        ...
        {voucher.priceDisplayMode !== "sem_valor" && (
          <section className="pdf-block-avoid border border-slate-200 rounded-lg p-4 bg-slate-50/50" data-pdf-block="true">
