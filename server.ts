...
const getStoreRef = () => {
  try {
    ...
    if (dbId) {
      return getFirestore(dbId).collection('system').doc('saas_store');
    }
  } catch(e) {
    console.warn("Could not use specific databaseId, falling back to default db");
  }
  return getFirestore().collection('system').doc('saas_store');
};
  ...
  try {
    ...
    if (supabase) {
      try {
        ...
        console.warn("Supabase fetch failed, falling back to local/firestore:", sbErr);
      }
    }
    ...
    try {
      const doc = await getStoreRef().get();
      if (doc.exists) {
        memoryStore = doc.data() as SaasStore;
        return memoryStore;
      }
    } catch (fsErr) {
      // Ignored if Firestore permission is restricted
    }
...
async function saveStore(store: SaasStore) {
  try {
    ...
    if (supabase) {
      try {
        await supabase.from('system_store').upsert({ id: 'main', data: store, updated_at: new Date().toISOString() });
      } catch (sbErr) {
        console.warn("Error syncing store to Supabase:", sbErr);
      }
    }

    // 3. Also try Firestore as backup
    try {
      await getStoreRef().set(store);
    } catch (fsErr) {
      // Ignored if permissions not set
    }
  } catch (err) {
...
app.post("/api/auth/invite", async (req, res) => {
  try {
    ...
    // Create user profile in Firestore via Admin SDK
...
app.get("/api/saas/agencies", checkMasterAuth, async (req, res) => {
  const store = await getStore();
  const agenciesWithCounts = (store.agencies || []).map((agency) => {
  ...
  try {
    if (firebaseApiKey) {
      ...
      if (data.error && data.error.message === "EMAIL_EXISTS") {
        ...
        // Password won't be sent if the user already existed and we couldn't create it
        generatedPassword = "";
        ...
        try {
          await getFirestore().collection("users").doc(data.localId).set({
            email: masterUser.email,
            role: "agency_admin",
            agencyId: agencyId,
          ...
          console.error("Could not write user profile to firestore:", fErr);
        }
      } else {
    ...
    console.error("Error creating Firebase Auth User via REST:", err);
  }
      ...
      await resend.emails.send({
        ...
        html: `
          <div style="font-family: sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
            ...
            <p><a href="https://vouchertur.com.br" style="display: inline-block; padding: 10px 20px; background-color: #0284c7; color: #fff; text-decoration: none; border-radius: 5px;">Acessar Painel</a></p>
          </div>
        `
      });
    } catch (emailErr) {
...
app.get("/api/vouchers", async (req, res) => {
  const agencyId = (req.query.agencyId as string) || (req.headers["x-agency-id"] as string);
  try {
    let query = getFirestore().collection("vouchers");
    let vouchersRef = query;
    
    if (agencyId) {
      vouchersRef = query.where("agencyId", "==", agencyId) as any;
    }
    ...
    console.warn("Firestore indisponível ou com erro na leitura de vouchers, usando armazenamento local:", (err as any)?.message);
    const store = await getStore();
...
app.post("/api/vouchers", async (req, res) => {
  ...
  try {
    ...
    await getFirestore().collection("vouchers").doc(newVoucher.id).set(newVoucher);
  } catch (err) {
    console.warn("Firestore indisponível para salvar voucher, salvando no repositório local:", (err as any)?.message);
    const store = await getStore();
