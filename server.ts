...
async function getStore(): Promise<SaasStore> {
  try {
    ...
    // 1. Try Supabase first if available (com timeout rápido de 1.5s para não atrasar inicialização)
    if (supabase) {
      try {
        const fetchPromise = supabase.from('system_store').select('data').eq('id', 'main').single();
        ...
        // Fallback gracioso imediato para o armazenamento local
      }
    }
...
async function saveStore(store: SaasStore) {
  try {
    ...
    // 2. Save to Supabase asynchronously in background (non-blocking)
    if (supabase) {
      (async () => {
        try {
